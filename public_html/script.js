// Ce fichier est telechargé et exécute par le navigateur au moment de 
// l'affichage de `index.html`.

// Cette ligne utilise jQuery pour trouver l'élément identifié par `search-bar`
// et enregistrer une action à exécuter à chaque fois que une touche est tapée
...
$("#search-bar").keyup(function() {
    ...
    // Lit le texte actuellement contennu dans la bare de recherche.
    var searchedText = $("#search-bar").val();
    // Decoupe le texte en une liste de mots
    var searchedWords = searchedText.split(/\W+/);
    
    var total = 0; // compteur du nombre total de video
    var visible = 0; // compteur du nombre de video corresmondant à la recherche

    // Utilise jQuery pour trouver les balises `li` situées sous l'élément 
    // identifié par `video-list`. Cela produit la liste des video. Un fonction 
    // est executée pour chacune des vidéo.
    $("#video-list li").each(function (elementPosition, element) {
        
        total++; // incrémente le total de video d'une unité
        ...
        // On passe ici quand auccun des mots de la recherche n'est présent.
        // La vidéo ne corespond pas à la recherche, il faut la cacher.
        $(element).hide();
    });// fin de la fonction appliquée à chaque video    

    // Ici l'inspection des videos est terminée.
    ...
    // Rends visible le bloc identifié par `no-video-message` si auccune vidéo 
    // n'est visible.
    $("#no-video-message").toggle(visible === 0);
    
});// fin de la fonction appelées à chaque changement de la bare de recherche.
